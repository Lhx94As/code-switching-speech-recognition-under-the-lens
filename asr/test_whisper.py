...
def main():
    ...
    if args.zeroshot is True:
        ...
        checkpoint_paths = glob.glob(f"ckpt_{args.dataset}/*.pt")
        if args.average:
            ...
            test_wer = evaluate(model, test_data, test_text_list)
            torch.save(model, f"ckpt_{args.dataset}/averaged_model_wer_{test_wer:.3f}.pt")
            logging.info(f"evaluated using zero-shot, WER {test_wer}")
            # logging.info(f"evaluated using averaged model: WER {test_wer} Eng WER: {test_wer} Man CER: {test_wer}\n model saved as <averaged_model_wer_{test_wer:.3f}.pt>")
        else:
            ckpt_ = checkpoint_paths[0]
            model = torch.load(ckpt_)
            model.eval()
            test_wer = evaluate(model, test_data, test_text_list)
            logging.info(f"evaluated using zero-shot, WER {test_wer}")
            # logging.info(f"evaluated using checkpoint: {ckpt_}, WER {test_wer} Eng WER: {test_wer} Man CER: {test_wer}")
