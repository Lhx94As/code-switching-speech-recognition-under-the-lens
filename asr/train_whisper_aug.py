...
from transformers import WhisperForConditionalGeneration, WhisperFeatureExtractor, WhisperTokenizer, WhisperProcessor, get_scheduler
import logging
...
def main():
      ...
      parser.add_argument('--aug', type=str, default='aug_data')
      parser.add_argument('--aug_ratio', type=float, default=0.5)
      parser.add_argument('--dev', type=str, default='dev_data')
      ...
      aug_audio_list, aug_text_list = preprocess_files(aug_path)
      dev_audio_list, dev_text_list = preprocess_files(dev_path)

      # Combine augmented data with training data
      train_audio_list += aug_audio_list[:int(len(aug_audio_list) * args.aug_ratio)]
      train_text_list += aug_text_list[:int(len(aug_text_list) * args.aug_ratio)]
