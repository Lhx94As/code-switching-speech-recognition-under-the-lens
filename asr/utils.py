...
def preprocess_text_file(filename):
    """
    Process a text file where each line has the format: audio_name <lang_id> text
    
    Args:
        filename (str): Path to the text file
        
    Returns:
        list: List of text lines (text content only, without audio_name and lang_id)
    """
    text_list = []
    with open(filename, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            # Match pattern: audio_name <LANG> text
            # The format can have tab or space separator between audio_name and <LANG>
            match = re.match(r'^\S+\s+<[^>]+>\s+(.+)$', line)
            if match:
                text = match.group(1).strip()
                
                # Remove special tags like <SPK/>, <NON/>, etc.
                text = re.sub(r'<[^>]+/?>', '', text)
                
                # Remove punctuation marks (both English and Chinese)
                # This includes: . , ! ? ; : - _ ( ) [ ] { } " ' / \ | ` ~ @ # $ % ^ & * + = < >
                ...
                # If pattern doesn't match, log a warning but continue
                logging.warning(f"Line format not recognized, skipping: {line[:50]}...")
...
#       text_list = [' '.join(process_sample(text_line)).lower().strip() for text_line in trans_data]
#       return audio_list, text_list



def save_best_checkpoints(model, step, error_rate, loss, save_dir="checkpoints", ckpt_records=None, max_ckpts=3):
    """
